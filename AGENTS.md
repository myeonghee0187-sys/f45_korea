...
- HTML
- CSS
- JavaScript(바닐라, 빌드 도구 없음)
- 별도 패키지 매니저·번들러 없이 index.html/style.css를 직접 브라우저에서 엽니다.
- React, Vue, TypeScript, Tailwind는 추가하지 않습니다.
- GSAP/ScrollTrigger/Lenis/Swiper는 PRD.md 16장 조건을 만족할 때만 추가하며, 그 전에는 CSS와 기본 JavaScript로 구현합니다.
...
- 이 프로젝트에는 lint·테스트·build 명령이 없습니다(package.json 없음, 정적 HTML/CSS/JS).
- index.html을 브라우저(또는 로컬 서버)로 열어 개발자 도구 콘솔에 오류가 없는지 확인합니다.
- PRD.md 14·17장 기준대로 360px, 768px, 1280px에서 가로 스크롤·잘림·겹침이 없는지 확인합니다.
- 모든 버튼, 링크, 입력, 메뉴를 실제로 조작하고 키보드 Tab 순서·focus-visible을 확인합니다.
